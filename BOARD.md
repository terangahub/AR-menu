...
| S47-04 | `POST /api/dishes/[id]/scan` | `FAIT` | Reçoit vidéo ou photos, valide les bornes (20-300 images), crée le `ScanJob` avant l'appel KIRI pour tracer même un échec au démarrage, distingue le 403 (crédit insuffisant, renvoyé en 402) du 401 (clé invalide, renvoyé en 500). `tsc`/`lint`/`build` verts. |
...
| S47-05bis | Configurer le webhook dans le dashboard KIRI | `TODO` | URL de production confirmée par Mouhamed : `https://vorae-menu.vercel.app`. À enregistrer dans *Settings » Webhooks* sur kiriengine.app : callback `https://vorae-menu.vercel.app/api/webhooks/kiri`, secret à générer et stocker dans `KIRI_WEBHOOK_SECRET`. Un seul webhook pour tous les jobs du compte. |
| S47-06 | Garde-fous d'usage | `TODO` | Chaque scan coûte de l'argent réel côté KIRI. Compteur et limite par restaurant obligatoires dès la première version, jamais d'accès illimité non facturé. **Le coût réel par plat dépend de la question ouverte sur `fileFormat`**, que S47-05 est justement conçu pour trancher au premier scan réel : 1 $/plat si un appel suffit pour glb+usdz, 2 $/plat sinon. |
