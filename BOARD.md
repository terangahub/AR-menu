...
| S7-15 | Modèle 3D trop volumineux pour un envoi Cloudinary simple | `FAIT` | **Révélé par la reprise automatique de S7-14** : `File size too large. Got 89973380. Maximum is 10485760.` Le modèle produit par KIRI pèse environ 86 Mo, largement au-dessus des 10 Mo que le compte Cloudinary accepte en un seul envoi pour un fichier `raw`. `uploadBuffer` bascule désormais sur `upload_large_stream` (envoi fractionné en morceaux de 6 Mo, sous le plafond) au-delà de 8 Mo. **Reste une incertitude non tranchée** : si les 10 Mo sont un plafond de plan Cloudinary et pas seulement par requête, le fichier reconstitué échouera quand même une fois tous les morceaux reçus - seul un nouveau test réel le confirmera. Voir `D-04`. |
