...
| S47-02 | Modèle Prisma `ScanJob` | `TODO` | Statut, fournisseur, `externalJobId` (= `serialize` côté KIRI), média source, coût, message d'erreur. Sur le principe de `DishCaptureOrder`, pour le flux automatisé plutôt qu'humain. |
| S47-03 | Adaptateur `lib/scan3d.ts` | `TODO` | Interface `Scan3dProvider` sur le modèle de `lib/billing.ts`. Implémentation KIRI en premier (endpoints réels par algorithme et type de média, voir `docs/roadmap-ai-instant-3d.md` section 0) ; RealityScan 2.1 en plan B si besoin de changer sans réécrire les routes. |
| S47-04 | `POST /api/dishes/[id]/scan` | `TODO` | Reçoit la vidéo (30 s) ou les photos (20 à 300), appelle l'endpoint KIRI adapté à l'algorithme choisi, crée le `ScanJob` avec `externalJobId = data.serialize`. Se fier au champ `ok` de la réponse, pas à `code` (incohérent en test réel : `200` reçu là où la doc montre `0`). |
| S47-05 | `POST /api/webhooks/kiri` | `TODO` | Réception du `POST {status, serialize}`, validation du secret de signature (mécanisme exact à observer sur la première requête reçue), `getModelZip`, upload Cloudinary, mise à jour de `Dish`. Répondre HTTP 200 impérativement. Rétention de 3 jours côté KIRI (statut 4 = Expired) : traiter dès réception, ne pas différer. **Ne pas supposer que D-03 (conversion glb/usdz) est résolue avant d'avoir vérifié si un seul appel donne les deux formats ou s'il en faut deux.** |
| S47-05bis | Configurer le webhook dans le dashboard KIRI | `TODO` | Manuel, dans *Settings » Webhooks* sur kiriengine.app : URL de callback (nécessite un déploiement Vercel accessible publiquement, pas testable en local) et secret de signature à générer et stocker dans `KIRI_WEBHOOK_SECRET`. Un seul webhook pour tous les jobs du compte, `serialize` sert à retrouver le bon `ScanJob`. |
| S47-06 | Garde-fous d'usage | `TODO` | Chaque scan coûte de l'argent réel côté KIRI. Compteur et limite par restaurant obligatoires dès la première version, jamais d'accès illimité non facturé. **Le coût réel par plat dépend de la question ouverte sur `fileFormat`** (S47-05) : 1 $/plat si un appel suffit pour glb+usdz, 2 $/plat sinon. |
