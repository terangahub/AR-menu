...
| S6-03 | Refonte du menu public et de la fiche plat | `DEV` | Écran vu par le convive, le plus critique commercialement. **Grille refondue autour de la photo** (absorbe `S5-07`) : cliché en 4:3, voile dégradé pour que le nom reste lisible sur une assiette claire, prix en pastille, badge AR en cube, et repli élégant sur l'initiale du plat quand la photo manque. **Barre de filtres collée en haut**, catégories en défilement horizontal pour ne pas repousser les plats sous la ligne de flottaison sur un téléphone, filtre allergènes replié avec compteur. **Fiche plat** hiérarchisée : visualiseur AR au ratio des cartes pour éviter le saut visuel, allergènes traités en encart dédié plutôt qu'en ligne de détail (c'est une décision de santé), ingrédients et temps de préparation en tableau. Nouveaux utilitaires `surface-menu`, `photo-scrim`, `menu-aurora`, `menu-sticky-bar` : ceux de la landing sont en `white/0.06` et invisibles en mode clair, alors que le menu doit tenir dans les deux thèmes. **Correction chromatique après relecture de Mouhamed** ("trop de couleurs, trop de violets différents, je veux vibe dark, plus pro, plus brandé, épuré") : la première version faisait cohabiter trois violets sur le même écran, la pastille lavande `--primary` (#f0d8f0) du badge AR, du prix et de la catégorie active, la carte indigo `--card` (#181848) sous les photos, et le halo violet de l'en-tête. La règle est désormais **neutre sur toute la surface, le violet réservé à la seule réalité augmentée, le rouge aux seuls allergènes**. En sombre une carte est un voile blanc à 5 % sur le fond quasi noir et non un aplat indigo ; les pastilles posées sur les photos passent en verre sombre (`photo-chip`) ; l'état actif des filtres et la bascule de langue passent en encre pleine (`bg-foreground`) plutôt qu'en aplat de marque ; le halo devient un unique dégradé blanc très diffus, en sombre uniquement ; le voile des photos passe du noir violacé au noir neutre, qui ne déplace pas les couleurs de l'assiette. Le bouton d'activation AR par défaut de model-viewer est remplacé par un bouton maison (`ar-launch`) dans le slot `ar-button`. **Second passage après essai sur téléphone.** Le bouton allergènes partageait la ligne des catégories et recouvrait la dernière catégorie sur un écran étroit : les deux commandes sont séparées sur deux lignes, les catégories occupent toute la largeur, et le panneau allergènes affiche chaque exclusion barrée avec une croix pour la retirer sans rouvrir le panneau. Le nombre de plats masqués par les filtres est dit explicitement, sinon une carte raccourcie par un filtre se confond avec un restaurant qui propose peu de plats. **Vue liste ajoutée** (`S6-09`) et **bascule photo / modèle 3D sur la fiche plat** (`S6-10`). Reste à valider au rendu réel (`S6-06`). |
...
| S6-04 | Refonte de la coquille du dashboard | `DEV` | La navigation tenait dans une seule barre horizontale où six rubriques, le nom du restaurant, le thème et le compte se disputaient la largeur, sans jamais indiquer la page courante. **Colonne fixe de 248 px au-delà de 1024 px**, barre horizontale conservée en dessous (une colonne y mangerait la moitié de l'écran). Identité du restaurant en tête (logo, nom, ville), rubriques avec icônes et **état actif visible** (`DashboardNav`, comparaison par préfixe sauf pour la vue d'ensemble, qui vit à la racine et exige l'égalité stricte). Lien sortant vers le menu public mis à part, en bas : ce n'est pas une rubrique du dashboard, c'est la vue du convive. Nouveaux utilitaires `surface-panel` et `nav-item`, sur la même discipline que le menu public : en sombre, un panneau est un voile blanc à 4,5 % sur le fond quasi noir, jamais l'aplat indigo de `--card`. **Bogue corrigé au passage** : le variant `outline` du bouton était en `border-white/25`, une valeur héritée de la landing (qui force le sombre) donc **invisible sur fond clair**. Passé aux tokens sémantiques. |
| S6-05 | Refonte des écrans du dashboard | `DEV` | Chaque page redéfinissait ses propres `rounded-lg border border-border p-4`, avec des rayons, des espacements et des tailles de titre divergents. Trois briques communes décident désormais de tout : `PageHeader`, `Panel`, `StatTile`, `EmptyState` (`components/dashboard/ui.tsx`). Écrans repris : vue d'ensemble, plats, QR codes, analytics globales et par plat, facturation, fiche plat (`S6-02`), formulaires, paramètres (`S6-08`), panneaux de médias et de capture (`S7-11`, `S9-06`). **Corrections de fond, pas seulement d'apparence** : la recherche de plats n'interrogeait que le nom français, donc en anglais le restaurateur tapait le nom affiché à l'écran et n'obtenait rien ; les états vides ne distinguaient pas une carte encore vide (qui appelle un premier plat) d'une recherche sans résultat (qui appelle un filtre moins strict) ; la disponibilité d'un plat était une case à cocher dont l'étiquette changeait de texte selon l'état, illisible en colonne, remplacée par un interrupteur au libellé stable ; les variations de scans reposaient sur la seule couleur, désormais doublées d'une flèche et d'un signe ; photo et modèle 3D étaient affichés à des ratios différents sur la fiche plat, ce qui rendait impossible la seule comparaison qui compte pour le restaurateur. La liste des plats montre enfin la vignette du plat. |
| S6-13 | Retours de Mouhamed sur le dashboard et l'en-tête du menu | `DEV` | **Six points relevés en testant la preview sur téléphone.** (1) **Bogue d'affichage** : sur la liste des plats, les actions étant `shrink-0` sur une seule ligne, elles prenaient toute la largeur, le nom du plat se réduisait à sa première lettre et la pastille `Indisponible` débordait par-dessus l'interrupteur. Les actions passent sous le nom tant que la largeur ne suffit pas. (2) **En-tête du menu public** : le nom du restaurant partageait sa ligne avec la bascule de langue et le thème, donc n'avait plus que la moitié de la largeur et se cassait en deux lignes sous le logo. Les commandes montent sur leur propre ligne, le nom dispose de toute la largeur. (3) **Vue grille et liste sur la liste des plats du dashboard**, mémorisée dans le navigateur, sur le modèle du menu public. (4) **Le bouton Supprimer n'était pas reconnu comme un bouton** : c'était un `ghost`, donc du texte nu, pour la seule action irréversible de l'écran. Composant `DeleteButton` partagé, contour rouge et icône, identique sur les plats et les QR codes. (5) **Uniformisation chromatique** : le bouton primaire passe de l'aplat lavande `--primary` à l'encre pleine, l'interrupteur de disponibilité passe du vert au neutre, la fenêtre de progression du scan abandonne `bg-primary`, `bg-card` et `bg-muted`, et `.input` gagne le rayon des panneaux et un anneau de focus. (6) **Bascule de langue dans le dashboard** : le restaurateur devait réécrire `/fr` en `/en` dans la barre d'adresse. Le composant sort de `components/menu/` pour devenir commun. |
| S6-14 | Refonte de l'écran analytics | `DEV` | **"On dirait Excel"** : quatre colonnes de chiffres alignées, où répondre à "quel plat marche" demandait de lire chaque ligne et de comparer mentalement. Or la question est une question de **grandeur relative**, et une grandeur relative se lit à une longueur, pas à un nombre. Chaque plat porte désormais une barre proportionnelle au plat le plus vu, et le taux d'activation AR une jauge sur piste, qui rappelle que 100 % existe. Le tri passe du clic sur en-tête à des pastilles explicites. **Piège évité grâce à la méthode** : foncer les barres à mesure qu'elles s'allongent est un anti-pattern documenté, il double l'encodage de la longueur par la teinte sur des catégories qui n'ont aucun ordre naturel. Toutes les barres ont donc la même couleur. Le graphique de tendance est repris en même temps : tracé et infobulle en tokens sémantiques (l'infobulle par défaut de Recharts est une boîte blanche à texte noir, illisible en sombre), aplat sous la courbe, grille horizontale seule en trait plein, curseur de survol. |
| S6-15 | Finition des trois écrans restés bruts | `DEV` | **Formulaire de plat** : une pile ininterrompue de dix-huit champs de même poids, où rien ne distinguait le nom et le prix, sans lesquels un plat n'existe pas, des ingrédients en anglais, qu'on remplit un mois plus tard. Trois sections, dans l'ordre où un restaurateur les renseigne. Les allergènes passent de cases à cocher à des pastilles à bascule (une case cochée dans une grille de quatorze se repère mal) et **corrigent un défaut de fond : leurs libellés étaient toujours en français**, y compris dans une interface anglaise. La disponibilité sort de la grille et explique sa conséquence, puisqu'elle décide de la présence du plat en salle. **Panneau de médias** : le sélecteur de fichier natif est masqué au profit d'un bouton du produit qui affiche le nom du fichier choisi, et les URL de stockage brutes (deux cents caractères qui ne disent rien) cèdent la place à un simple état En place / Manquant. **Carton d'impression QR** : de trois lignes centrées à un vrai carton de table, avec logo, numéro de table, marge blanche autour du code (la *quiet zone* du standard, sans laquelle un lecteur peine sur une nappe foncée) et consigne pour le convive. Il est **toujours noir sur blanc** : imprimé depuis le mode sombre, il sortait en blanc sur fond sombre, soit une page entière d'encre. |
| S6-16 | Impression réellement noire sur blanc, et adresse du menu qui débordait | `DEV` | **Correction d'une correction ratée.** `S6-15` annonçait un carton d'impression toujours noir sur blanc ; en réalité seule la carte était forcée en blanc, jamais la page. Le `body` porte `bg-background`, soit #030014 en sombre : la feuille sortait entièrement noire avec un timbre blanc au milieu, exactement ce que le projet n'avait aucune règle `@media print` pour empêcher. Bloc d'impression global ajouté (`html`/`body` en blanc, `print-color-adjust: exact` pour que le navigateur ne supprime pas cet aplat, `@page { margin: 12mm }`). Le gabarit est repris en même temps : le carton occupait un timbre au centre d'une feuille vide, il remplit désormais la page, le QR code est dimensionné en millimètres et non en pixels pour sortir à une taille scannable, la consigne passe avant le code (dire quoi faire, puis donner la cible) et le cadre devient un pointillé, qui se lit comme un trait de coupe. **Second point** : dans les paramètres, le domaine était collé devant le champ d'adresse en `shrink-0`, si bien qu'une URL de preview Vercel poussait la saisie hors de l'écran. Le champ est seul, l'adresse complète s'affiche dessous où elle peut revenir à la ligne. |
| S6-17 | L'adresse gravée dans les QR codes suivait le déploiement, pas le produit | `DEV` | **Question de Mouhamed en voyant `ar-menu-g082aq340-...` dans les paramètres.** `NEXT_PUBLIC_APP_URL` étant vide sur Vercel, `absoluteMenuUrl` retombait sur `VERCEL_URL`, qui est l'adresse **d'un déploiement** et change à chaque `git push`. Le chemin stocké en base est relatif, donc portable ; mais l'URL absolue est **gravée dans l'image** du QR code au moment de la génération, et un carton collé sur une table ne peut plus être corrigé. Un QR imprimé aujourd'hui porterait donc une adresse jetable, non conforme à la marque, qui casserait le jour où la protection de déploiement serait activée. La résolution passe à `NEXT_PUBLIC_APP_URL`, puis `VERCEL_PROJECT_PRODUCTION_URL` **en production uniquement** (l'alias stable du domaine, celui dont Next.js se sert lui-même pour `metadataBase`), puis `VERCEL_URL` en preview, pour qu'un QR généré pendant un test ne renvoie pas le testeur vers la production. |
...
| S6-08 | Paramètres du restaurant dans le dashboard | `DEV` (non stylé) | **Manque signalé par Mouhamed : le nom du restaurant et son logo n'étaient modifiables qu'en base de données.** Un restaurateur qui change d'enseigne, corrige une faute dans son nom ou refait son logo n'avait aucun recours dans le produit. Nouvel écran `/dashboard/settings` : nom, ville, courriel de contact, langue d'accueil, logo (envoi et retrait), et adresse du menu. `PATCH /api/restaurant` et `POST`/`DELETE /api/restaurant/logo`. **L'adresse du menu est le point délicat** : elle est encodée dans l'image des QR codes déjà collés sur les tables, qu'aucune mise à jour à distance ne peut corriger. Trois mesures : les `targetUrl` en base sont réécrits dans la même transaction que le changement (pour les réimpressions), un avertissement explicite s'affiche dès que l'adresse est modifiée, et la page de menu rattrape les anciens QR codes via leur `?qr=` (voir `S6-11`). La langue d'accueil, jusqu'ici une colonne morte, pilote désormais réellement la locale encodée dans les QR codes (`lib/qr-target.ts`, partagé entre la génération et la mise à jour). |
| S6-09 | Vue liste sur le menu public | `DEV` | **Demandé par Mouhamed** : une carte de brasserie peut compter quarante plats, la grille en photos oblige alors à un défilement interminable. Bascule grille / liste dans la barre de filtres, mémorisée dans le navigateur du convive (`localStorage`, lu après montage pour ne pas casser l'hydratation). La vue liste garde une vignette carrée de 68 px, le nom et le prix sur une ligne, la description coupée à une ligne, les allergènes en texte : dix plats à l'écran là où la grille en montrait deux. |
| S6-10 | Bascule photo / modèle 3D sur la fiche plat | `DEV` | **Signalé par Mouhamed : en ouvrant un plat, la photo disparaît.** Le modèle 3D la remplaçait purement et simplement, alors que c'est la photo qui donne faim, que c'est elle que le restaurateur a payée, et qu'un maillage de photogrammétrie est souvent moins flatteur qu'un cliché professionnel. Segmenté `Modèle 3D` / `Photo` sous le visualiseur, modèle 3D par défaut (c'est ce que le convive vient chercher en touchant une carte marquée du cube). Le visualiseur est **masqué en CSS et non démonté** : un modèle pèse plusieurs dizaines de mégaoctets, le démonter le ferait retélécharger à chaque aller-retour. |
| S6-11 | Rattrapage des QR codes imprimés après un changement d'adresse | `DEV` | Corollaire de `S6-08`. Un QR code collé sur une table encode l'ancienne adresse dans son image : après un renommage, il menait à un 404, devant un convive attablé. Le `?qr=` qu'il porte identifie pourtant le QR code de façon stable. La page de menu s'en sert pour retrouver le restaurant et rediriger vers son adresse actuelle. |
| S6-12 | Lien vers la landing depuis le menu public | `DEV` | **Demandé par Mouhamed.** La mention `Menu propulsé par Vorae` est le seul point de contact commercial de tout le menu, et n'était pas cliquable : un convive intrigué n'avait aucun moyen d'aller voir ce qu'est Vorae. Le mot `Vorae` devient un lien vers la landing, ouvert dans un nouvel onglet pour ne pas faire perdre au convive sa place dans le menu ni le rattachement de sa visite à sa table. Voir `S8-09` pour le retrait de cette mention au palier Prestige. |
...
| S8-09 | Marque blanche au palier Prestige | `TODO` | **Écart relevé en rendant cliquable la mention `Menu propulsé par Vorae` (`S6-12`)** : cette mention s'affiche aujourd'hui sur tous les menus, quel que soit le palier, alors que `Restaurant.whiteLabelParentId` existe dans le schéma depuis le Sprint 0 et n'est lu nulle part. Rien dans le code ne relie encore un palier à un retrait de la mention. À trancher avec la section 15.1 : la marque blanche est-elle incluse au palier Prestige (449 $/mois) ou vendue à part ? |
| S8-11 | Gestion des catégories de la carte | `TODO` | **Question posée par Mouhamed.** Aujourd'hui la catégorie est un simple champ texte sur chaque plat (`Dish.category` / `categoryEn`), et la liste affichée est déduite des plats existants. Conséquences : **créer** une catégorie marche déjà (il suffit de la taper sur un plat) ; **supprimer** n'a pas de sens, une catégorie disparaît d'elle-même quand plus aucun plat ne la porte ; mais **renommer** oblige à rouvrir chaque plat un par un, et **l'ordre d'affichage sur le menu public est celui de création des plats**, donc arbitraire. Ce dernier point est le vrai manque : un restaurateur veut Entrées, puis Plats, puis Desserts, pas l'ordre dans lequel il a saisi sa carte. Demande un modèle `Category` (ou au minimum une liste ordonnée sur `Restaurant`) et une migration des valeurs texte existantes, d'où le classement en Sprint 8 plutôt qu'un ajout à chaud. |
| S8-10 | Couleur de marque du restaurant sur son menu | `TODO` | `Restaurant.primaryColor` est en base depuis le Sprint 0 et exposé par `GET /api/menu/[restaurantSlug]`, mais aucun écran ne l'utilise : un restaurateur ne peut pas teinter son menu à ses couleurs. À reprendre avec la discipline chromatique du menu (voir `CONTEXT.md`) : la couleur du restaurant ne doit pas réintroduire le bariolage que `S6-03` a supprimé. Champ volontairement absent de l'écran de paramètres (`S6-08`) tant qu'il n'a aucun effet : un réglage qui ne fait rien est pire que pas de réglage. |
...
7. **Renseigner `NEXT_PUBLIC_APP_URL`** dès qu'un vrai domaine est branché, puis **redéployer** (les variables d'environnement sont figées à la construction). Sans ça, les QR codes générés en production porteront l'alias Vercel par défaut plutôt que le domaine de marque. **Et réimprimer les QR codes générés avant ce changement** : leur ancienne adresse est gravée dans l'image, aucune mise à jour à distance ne peut la corriger (voir `S6-17`).
8. **Régénérer le jeton du store Blob** (`BLOB_READ_WRITE_TOKEN`) : il a été collé en clair dans une conversation le 27 août. Rien n'indique une fuite, mais un jeton de lecture-écriture qui a quitté son environnement doit être considéré comme compromis.
