...
Sprint 7 (capture 3D) en test réel de bout en bout.
...
| Sprint 7 | Capture 3D automatisée des plats (KIRI Engine) | `DEV` | - |
...
| S5-07 | Afficher les photos de plats sur la grille du menu public | `TODO` | Découvert en vérifiant l'`UPDATE` Neon : la grille du menu n'affiche que le texte et le prix, jamais `Dish.imageUrl`. Les photos ne sont donc visibles que sur la fiche plat. Repris dans le Sprint 6. |
...
| S6-03 | Refonte du menu public et de la fiche plat | `TODO` | Écran vu par le convive, le plus critique commercialement. Absorbe S5-07 (photos sur la grille). |
...
| S7-13 | Test réel sur le restaurant pilote | `TODO` | Avec les crédits gratuits de S7-01, sur de vrais plats, pas des photos de stock. Condition de passage à l'échelle. |
...
| S7-15 | Modèle 3D trop volumineux pour un envoi Cloudinary simple | `BLOQUÉ` | **Révélé par la reprise automatique de S7-14** : `File size too large. Got 89973380. Maximum is 10485760.` Le modèle produit par KIRI pèse environ 86 Mo, largement au-dessus des 10 Mo que le compte Cloudinary accepte. Deux correctifs tentés côté code, tous deux insuffisants. (1) `upload_large_stream`, qui existe dans les types du SDK mais pas réellement sur `v2.uploader` dans cette version (2.10.1) - confirmé à l'exécution (`typeof upload_large_stream === "undefined"`), corrigé vers `upload_chunked_stream`, la fonction réellement exposée. (2) Envoi fractionné en morceaux de 6 Mo (sous le seuil) : nouvel échec, mais avec `Got 12582912` - exactement deux morceaux de 6 Mo cumulés, pas la taille d'un seul morceau. **Conclusion tranchée, pas une hypothèse** : les 10 Mo sont un plafond sur la taille totale du fichier reconstitué, pas sur chaque requête. Le découpage ne peut donc pas contourner cette limite, quelle que soit la taille des morceaux. Aucun correctif côté code ne peut résoudre ça : il faut soit relever la limite de fichier `raw` dans les réglages ou le plan du compte Cloudinary (action Mouhamed, à vérifier en premier - rapide si le plan le permet), soit migrer le stockage des modèles 3D ailleurs (`D-04`, plus lourd - implique un compte AWS S3 à créer, aucun n'existe actuellement). |
...
| D-04 | Modèles 3D sur Cloudinary au lieu d'AWS S3 + CloudFront | `BLOQUANT` | **Confirmé, plus seulement théorique** (voir `S7-15`) : le premier modèle réel produit par KIRI pèse environ 86 Mo, et le plafond de 10 Mo du compte Cloudinary porte sur la taille totale du fichier `raw`, pas sur chaque requête - un envoi fractionné en petits morceaux ne le contourne donc pas. Tant que ni le plan Cloudinary n'est relevé ni le stockage migré, aucun modèle KIRI de cette taille ne peut être conservé. Bloque la suite du Sprint 7 (S7-13, le pilote réel) : les vrais plats produiront très probablement des modèles de taille comparable. |
...
5. **Vérifier le plafond de taille de fichier `raw` du compte Cloudinary** (`D-04`, `S7-15`). Le premier modèle 3D réel (environ 86 Mo) est refusé, le compte n'acceptant que 10 Mo par fichier `raw`, et ce plafond porte sur la taille totale du fichier, pas sur chaque requête - aucun correctif côté code ne peut le contourner. À chercher dans le tableau de bord Cloudinary, section réglages d'upload ou limites du plan. Si le plan actuel ne permet pas de le relever, la seule autre option est de migrer le stockage des modèles 3D ailleurs (S3, plus lourd, aucun compte AWS n'existe actuellement). Bloque le reste du Sprint 7.
