...
| S7-18 | Modèle KIRI trop lourd pour un téléphone | `TODO` | **Constaté en test réel** : le `.glb` de 86 Mo fait planter Safari sur iPhone (« A problem repeatedly occurred ») au chargement de la fiche. Un modèle de photogrammétrie brut n'est pas exploitable tel quel sur mobile, alors que c'est précisément la cible du produit. À traiter avant tout pilote réel : décimation du maillage et compression des textures (Draco/KTX2) pour viser quelques Mo, soit dans les paramètres KIRI (`modelQuality`, `textureQuality`, réglés au plus haut aujourd'hui), soit en post-traitement. Bloque `S7-13`. |
...
5. **Créer et connecter un store Vercel Blob** (`D-04`, `S7-15`). Le premier modèle 3D réel (environ 86 Mo) était refusé par Cloudinary (plafond de 10 Mo par fichier `raw`, y compris sur le plan payant à 99 $/mois qui plafonne à 20 Mo) : les modèles 3D migrent vers Vercel Blob. Dans le tableau de bord Vercel : Storage > Create Database > Blob, type **public** (ne peut plus changer ensuite), puis onglet Projects du store > Connect to Project, sur `ar-menu`, environnements Preview et Production. Aucune variable à copier à la main, l'authentification se fait par OIDC à la connexion. Bloque le reste du Sprint 7 tant que ce n'est pas fait.
