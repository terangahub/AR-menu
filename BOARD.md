...
| S7-15 | Modèle 3D trop volumineux pour un envoi Cloudinary simple | `FAIT` (2 essais) | **Révélé par la reprise automatique de S7-14** : `File size too large. Got 89973380. Maximum is 10485760.` Le modèle produit par KIRI pèse environ 86 Mo, largement au-dessus des 10 Mo que le compte Cloudinary accepte en un seul envoi pour un fichier `raw`. Premier correctif basé sur `upload_large_stream`, qui existe dans les types du SDK mais pas réellement sur `v2.uploader` dans cette version (2.10.1) - défini dans `lib/uploader.js`, jamais reporté vers l'API v2 publique - d'où un nouvel échec réel (`r is not a function`) au tour suivant de la reprise automatique. Confirmé à l'exécution (`typeof upload_large_stream === "undefined"`) avant de corriger vers `upload_chunked_stream`, la fonction réellement exposée, avec la même convention d'appel. **Reste une incertitude non tranchée** : si les 10 Mo sont un plafond de plan Cloudinary et pas seulement par requête, le fichier reconstitué échouera quand même une fois tous les morceaux reçus - seul un nouveau test réel le confirmera. Voir `D-04`. |
...
| D-04 | Modèles 3D sur Cloudinary au lieu d'AWS S3 + CloudFront | `TODO` | **Devenu concret, pas seulement théorique** : le premier modèle réel produit par KIRI (S7-12) pèse environ 86 Mo, largement au-dessus des 10 Mo que le compte Cloudinary accepte en un seul envoi (`File size too large`). Contourné en S7-15 par un envoi fractionné (`upload_chunked_stream`, morceaux de 6 Mo), mais si les 10 Mo sont un plafond de plan (pas seulement par requête), le fichier reconstitué échouera quand même à la fin - seul un vrai test le dira. Si ça persiste, la bascule vers S3 cesse d'être une amélioration future et devient bloquante. |
